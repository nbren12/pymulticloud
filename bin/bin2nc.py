...
def my_dtype(head):
    ...
    mydt = np.dtype([('time', np.float64),
                     ('u', np.float64, (n, 1, ntrunc)),
                     ('th', np.float64, (n, 1, ntrunc)),
                     ('q', np.float64, (n, 1)),
                     ('teb', np.float64, (n,1 )),
                     ('hc', np.float64, (n,1 )),
                     ('hd', np.float64, (n,1 )),
...
def output2xarray(head, data):
    ...
    for field in data.dtype.fields:
        if field == 'time':
            pass
        elif data[field].shape == (t.shape[0], x.shape[0], 1, m.shape[0]):
            dat = np.transpose(data[field], axes=(0,3,2,1))
            xrdict[field] = (['time', 'm', 'y', 'x'], dat)
        elif data[field].shape == (t.shape[0], x.shape[0], 1):
            dat = np.transpose(data[field], axes=(0,2,1))
            xrdict[field] = (['time', 'y', 'x'], dat)
