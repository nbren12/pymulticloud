subroutine multicloud_wrapper(fc, fd, fs, u1, u2, t1, t2, teb, q, hs, n, dt,&
  ...
  integer :: n
  real(8), intent(in) :: tebst(n)
  real(8), intent(inout) ::  u1(n),u2(n),t1(n),t2(n),q(n),teb(n),hs(n),&
       fc(n), fd(n), fs(n)

  real(8), intent(in) :: dt, dx, time
  real(8), intent(out) :: hc(n), hd(n), moiststab(n)
...
subroutine multicloud_eq(fceq, fdeq, fseq)
  use util
  use param_mod
  use multicloud_mod, only: init_multicloud, updatehcds, range_kuttas, get_eqcloudfrac, T, L, alpha_bar, c, ud, theta_ebs_m_theta_eb
  ...
  !     PROGNOSTIC VARIABLES
  real(8), intent(out) ::  fceq, fdeq, fseq
