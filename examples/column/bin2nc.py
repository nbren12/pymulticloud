...
color_blind = {
    'axes.prop_cycle'    : "cycler('color', 'bgk')"
}

mpl.rcParams.update(color_blind)
...
def column_report(name):
    ...
    xr['sh'].plot(ax=axs[5])
    xr['scd'].plot(ax=axs[5])
    xr['sdry'].plot(ax=axs[5])
...
def columnbin2csv(name):
    """Process column.bin to a csv file"""
    xr = output2xarray(*read_output(name))
    df = xr.drop('m').to_dataframe()
