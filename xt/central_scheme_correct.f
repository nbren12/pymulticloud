...
C
C   NON OSCILLATORY CENTRAL SCHEME OF G-S JIANG AND EITAN TADMOR
C
...
c
c     second order central scheme
c
c
c     staggered cell averages reconstruction:   xi_stag(i,j):=xi(x_{i+1/2},y_{j+1/2})
c
      do J=1,5
         DO I=1,N
...
c
c     predictor: mid-time-step pointwise values at cell-center-grid-points (x_i)
c
     ...
     X    Q_TLD)* UX(1,I) 
     X        + (alpha_tld*UC(5,I)*NLT +
     x        lmd_tld*Q_TLD)* UX(2,I) 
     x      +  (UC(1,I)*NLT + alpha_tld*UC(2,I))*UX(5,I) 
     X        )
...
c
c     boundary conditions for mid-time-step cell-centered values
c
...
c
c     Corrector: high resolution second order scheme 
c
     ...
     x        UC(5,I+1)*(UC(1,I+1) + ALPHA_TLD*UC(2,I+1))*NLT + 
     x        Q_TLD*(UC(1,I+1) + LMD_TLD*UC(2,I+1))
     x        -UC(5,I)*(UC(1,I)+ ALPHA_TLD*UC(2,I))*NLT 
     x        - Q_TLD*(UC(1,I) + LMD_TLD*UC(2,I)))
         
      ENDDO
          
c
c     Boundray conditions and slopes for staggered values
c
...
c
c    High resolution slopes calculation 
c     
     ...
     x               (uc(j,i+1)-uc(j,i-1))/2,tht*(uc(j,i)-uc(j,i-1)))
