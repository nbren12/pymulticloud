...
! stochatic arrays
integer scmt(1:n) ! CMT state

! cloud fraction
real(dp) :: fcls(1:n), fdls(1:n), fsls(1:n)
...
if ( BINARY_OUTPUT) then
   ...
   open(unit=ufid, file='column.bin', form='unformatted', status='NEW', access='stream')
   write (ufid) n
   write (ufid) ntrunc
   write (ufid) dx*L
   write (ufid) T
   write (ufid) L
   write (ufid) alpha_bar
else
   OPEN(UNIT=SNAPSHOT_ID,FILE='snap_shots',STATUS='unknown')
endif
